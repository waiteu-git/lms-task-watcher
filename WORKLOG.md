...
- **パス型決済（半期¥720・年¥1,200）はv1.2.0から除外**しバックログへ。リタス初版公開（9月）後に優先度再評価。月額¥120は継続。7/4 free-firstスペックの決済記述を本決定で凍結
- 保存はローカルのみ・**バックエンド変更なし**（ラズパイのデプロイ不要）
- **コード流通の規約**: 純粋ロジック（パーサ・差分・シラバスURL生成）は共有パッケージ化せず、**拡張で実DOM検証して磨き→litasへ逆流**。litasに既存実装があるため単純上書きせず突き合わせて寄せる。対応表（拡張↔litasパス）をスペックに明記。逆流状況はWORKLOG/handoverに記録する運用
