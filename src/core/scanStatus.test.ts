...
import { rearmDeadlineNotifications, getNotifiedDeadlineKeys, saveNotifiedDeadlineKeys } from './scanStatus'

describe('rearmDeadlineNotifications', () => {
  it('指定idに紐づく通知済みキーだけを取り除く', async () => {
    await saveNotifiedDeadlineKeys(['a:1h', 'a:24h', 'b:24h'])
    await rearmDeadlineNotifications('a')
  ...
  it('区切り文字込みの前方一致のため、部分一致するidを誤って削除しない（m1 と m10）', async () => {
    ...
    await rearmDeadlineNotifications('m1')
  ...
  it('通知済みキーが未設定でも例外を投げない', async () => {
    await expect(rearmDeadlineNotifications('a')).resolves.toBeUndefined()
  })
})
