...
describe('clearDeadlineOverride', () => {
  it('正規化キーで該当分だけ削除し他は保持する', async () => {
    const store: Record<string, unknown> = {
      deadlineOverrides: {
        'https://letus.ed.tus.ac.jp/mod/assign/view.php?id=1': '2026-07-30T14:59:00.000Z',
        'https://letus.ed.tus.ac.jp/mod/quiz/view.php?id=9': '2026-07-01T00:00:00.000Z',
    ...
    const { clearDeadlineOverride } = await import('./deadlineOverride')
    await clearDeadlineOverride('https://letus.ed.tus.ac.jp/mod/assign/view.php?id=1#anchor')
