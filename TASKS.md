...
- [~] **旧マーク（Vite稲妻＋Vite由来の紫）の取りこぼし洗い出し（2026-09-25・統合ハブ依頼）**
  ...
  - 🔴**LTWリポ（PUBLIC）に旧マークのPNGが2枚残存**（7/2以降無編集・参照元なし・生成元なし）: `store-assets/product-final.png`（512²・Stripe「Premium」商品画像・巨大な稲妻）／`store-assets/stripe-logo.png`（680×128・稲妻＋「LETUS Task Watcher」）。9/11の差し替え対象表は`stripe-icon.png`だけを数えていて、この2枚が漏れていた＝「同じ資産が何製品に入っているか」の数え漏れ。Premium/Stripeは休眠中（`VITE_API_BASE_URL`ゲート）で実害の露出は小さいが、追跡中の公開ファイルなので**本人へ処置を諮る**（案: 追跡から外す／T-markで再生成）。Stripeダッシュボード側にアップロード済みかは私からは確認できない
