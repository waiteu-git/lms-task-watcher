...
- [x] **統合ハブ横断監査（2026-09-18・10エージェント）でLTW関連4件、独立検証のうえ対応**
  - ✅**①runAutoScanの偽成功記録**: `runManualUpdate`は失敗時に`lastSuccessfulRefreshAt`更新をスキップし「偽の成功を作らない」と明記しているのに対し、`runAutoScan`は結果に関わらず常に更新していた実バグ。diagnosticsStateが別経路で既に検知しているのではと疑って確認したが、**diagnosticsStateはネットワーク障害等を意図的に「中立」扱いし記録しないため2つの安全網は補完関係**＝runAutoScan側の不具合でポップアップに依存しない唯一の能動的stale検知（App.tsxの経過時間チェック）が実質機能停止していたと確認。`runManualUpdate`と同じ契約に揃えて修正、再現テスト付きでコミット済み（`2b867ca`・745テスト緑・tsc/lint/build成功）。**ユーザー裁定＝次回の機能リリースに併設**（単独パッチ出しはしない）
  - ✅**②transparency.htmlの技術的誤り**: 「host_permissionsにより技術的に他ドメインへ送信不可能」はMV3の実際の仕様と食い違う（host_permissionsが制御するのは主にクロスオリジンのレスポンス読み取り・コンテンツスクリプト注入で、送信自体を塞ぐものではない）。`public-profile-claims-need-repo-proof`の同型4件目。ユーザー確認のうえ「ソースコード全文公開による検証可能性」に差し替えてコミット済み（`54a813d`）。版数表記（v1.4.2のまま）は別件で「対象外・据え置き」済みにつき今回は不可侵
  - ✅**③実機確認未チェックのまま出荷**: `store-submission-v1.4.1.md:30`・`v1.4.2.md:29`とも「実機確認」が`- [ ]`のまま両ストアへ出荷済みと確認。遡ってチェックを付けることはせず、正直に「未実施のまま出荷」の記録として残す。要求する検証（CLASS実ログインでの学期表示挙動）は9/11の実機観測タスク（下記）と同根の制約＝ユーザー本人にしかできない
  - ⏸️**今後の運用方針（本人未確認・統合ハブの指示によりMichizureデプロイ完了の合図まで保留）**。選択肢だけ用意しておく：**A=実機確認を必須ゲート化**（確認が取れるまで提出しない・品質最優先でリリース速度は犠牲）／**B=リスク別サンプリング**（影響範囲に応じて必須/スキップ可を項目ごとに仕分け、軽微なものは未確認のまま出す代わりに正直にログする）／**C=チェックリストから分離**（要実機確認の項目は別トラックで可視化を続けるが、通常のstore-submission提出ゲートには含めない＝提出は止めない）
...
- [~] **統合ハブ公開面棚卸し（2026-09-25・リタス両OS製品版配信開始に伴うLTW側の古い記述6件）**＝独立検証のうえ対応中
  ...
  - ✅**4 mainの古いchangelog／manifest(1.4.0)＝過小な記述。実態は「公開mainに旧マーク一式（Viteロゴ由来の稲妻＋Vite色`#863bff`）が残っている＝商標の是正でもある」**: 公開main(`fa6cb6b`)は旧マークのファイルを残したまま（統合ハブの未認証実測＋私のgrep=テキスト13本にシグネチャ・favicon/アイコンPNG/logo-300x300/store-promo等のPNGを含め約29ファイル・未認証curlで200）。⚠**ローカル`~/dev/lms-task-watcher-main`のgrepは「綺麗」と誤判定する**（ローカルは`a00deff`まで進むが、そのコミットは公開リポに存在しない＝判定は未認証curl／`git ls-remote`＋`origin/main`で）。developの現ツリーをそのまま採る再同期コミットを作成済み（親=旧同期`a00deff`+develop。**developへコミットを足すたびに作り直す＝ブランチ`sync-main-candidate`に保持・承認依頼時に現行SHAを確認**）。mainへの反映＝`git push origin <SHA>:refs/heads/main`（ローカルmainのref移動を避けられる）は本人承認待ち。manifest 1.4.0→1.4.3も同時に揃う
...
- [~] **旧マーク（Vite稲妻＋Vite由来の紫）の取りこぼし洗い出し（2026-09-25・統合ハブ依頼）**
  ...
  - 追跡中テキストの`#863bff`はTASKS.mdとstore-submission-v1.4.3.mdの記述（是正の記録）のみ・稲妻パスのシグネチャは0件
...
- [ ] **2026-09-11（後期開始日）に実機観測する**＝日付の一回性タスク（§8-⑥: 条件に紐づけると無言で落ちるので日付で持つ）
  ...
  - 🔴**2026-09-23時点の結論＝取れなかった。** 統合ハブの棚卸しで未記録と指摘され確認したが、上記CLASSログイン必須の項目（後期時間割表・pref保持・課題カードの曜限/教室/シラバス）はTASKS.md上もそれ以外の記録場所にも一切残っていない＝本人は実施していない、または実施したが記録されなかったかのいずれか判別不能。**学期境界の観測機会（年1回・後期開始日）は2026年分はもう過ぎている**ため今回分は取得不能で終了。次の機会は2027年の後期開始日（この時点で学期境界のLitusとの食い違い＝統合ハブ所管・未裁定の材料にできるかは、その時まで持ち越し）
