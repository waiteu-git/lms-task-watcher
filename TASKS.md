...
- [~] **アイコン刷新（Vite由来の意匠・配色を置き換え）＝実装完了・残るはユーザー承認後のpushとストア申請のみ**（2026-09-10開発本部回付・2026-09-11意匠確定・同日実装）
  ...
  - ✅Edge Partner Center 下書き提出（2026-09-11〜12・ユーザー指示「販促タイルも直す」への対応込み）:
    ...
    - 大型プロモタイル（`promo-marquee-1400x560.png`）とスクリーンショット3枚（`store-shot1/2/3.png`）は**未確認・未対応のまま**。旧意匠を含むかどうか目視確認できていない＝次回持ち越し
  ...
  - 残タスク: ①Edge側「公開」ボタン（未クリック・ユーザーの明示的な承認が要る不可逆操作）②大型プロモタイル・スクリーンショット3枚の目視確認と必要なら差替え ③Chrome Web Store側の手動提出一式（ユーザー操作） ④完了後にSHAと両ストア提出状況を開発本部（`local_2a8a2a05-d74b-4051-a17c-c8f34294d294`）へ報告
